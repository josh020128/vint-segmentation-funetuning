...
def demo(model):
    rgb = np.array(Image.open("src/mrvn/mrvn/UniDepth/assets/demo/rgb.png"))
    rgb_torch = torch.from_numpy(rgb).permute(2, 0, 1)
    intrinsics_torch = torch.from_numpy(
        np.load("src/mrvn/mrvn/UniDepth/assets/demo/intrinsics.npy")
    )
    ...
    depth_gt = (
        np.array(Image.open("src/mrvn/mrvn/UniDepth/assets/demo/depth.png")).astype(float)
        / 1000.0
    )
    ...
    Image.fromarray(artifact).save("src/mrvn/mrvn/UniDepth/assets/demo/output.png")
